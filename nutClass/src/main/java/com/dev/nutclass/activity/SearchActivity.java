...
public class SearchActivity extends BaseActivity implements OnClickListener{
	...
	private void initView() {
		...
		clearIv = (ImageView) findViewById(R.id.iv_clear);
		...
		hotLayout=(LinearLayout) findViewById(R.id.ll_hot);
		catLayout=(LinearLayout) findViewById(R.id.ll_cat);
	...
	private void initIntent() {
		 ...
		 if(getIntent().hasExtra(Const.KEY_KEYWORD)){
			keyword=getIntent().getStringExtra(Const.KEY_KEYWORD);
		 }
	}

	private void initData() {
		inputEdit.setText(keyword);
		if(TextUtil.isNotNull(keyword)){
			updateHot(null);
	...
	public void onClick(View v) {
		...
		if(v==backIv){
			...
			keyword=inputEdit.getText().toString();
			if(TextUtils.isEmpty(keyword)){
				DialogUtils.showToast(context, "请输入搜索关键字");
				return;
			}
			Util.hideSoftInput((Activity)context, inputEdit);
			updateHot(null);
			reqData(1);
		}else if (leftLayout == v) {
			if (showFilter == 1) {
				showFilter = 0;
			} else {
				showFilter = 1;
	...
	private void reqData(final int page) {
		...
		String url =  String.format(HttpUtil.addBaseGetParams(UrlConst.SEARCH_URL), keyword);
		url=url+"&pageNo="+page;
		url=url+"&school_id="+cid;
		url=url+ "&district_id=" + distinctId + "&order_price="
				+ priceId + "&order=" + filter + "&age_id=" + age;
		OkHttpClientManager.getAsyn(url,
				new OkHttpClientManager.ResultCallback<String>() {
					...
					public void onResponse(String response) {
						...
						refreshLayout.finishRefreshLoadMore();
						refreshLayout.finishRefresh();
						BaseParser<BaseCardEntity> parser = new CardListParser();
	...
	private void reqHot() {
		...
		String url =  HttpUtil.addBaseGetParams(UrlConst.SEARCH_HOT_URL);
		...
		OkHttpClientManager.getAsyn(url,
				new OkHttpClientManager.ResultCallback<String>() {
					...
					public void onResponse(String response) {
						...
						ArrayList<BaseCardEntity> list =null;
						try {
							retObject = new JSONObject(response);
							
							int result=retObject.optInt("status",0);
							
							if (result== UrlConst.SUCCESS_CODE) {
								JSONArray jsonArray=retObject.optJSONArray("data");
								
								if(jsonArray!=null&&jsonArray.length()>0){
									list = new ArrayList<BaseCardEntity>();
									for(int i=0;i<jsonArray.length();i++){
										SingleItemCardEntity item=new SingleItemCardEntity();
										item.setDesc(jsonArray.optString(i));
										list.add(item);
									}
								}							
							}
	...
	@SuppressLint("NewApi")
	private void updateHot(List<BaseCardEntity> list){
		if(list!=null&&list.size()>0){
			...
			for (int i = 0; i < list.size(); i++) {
				LinearLayout.LayoutParams cellParams = new LinearLayout.LayoutParams(DensityUtil.getDisplayWidth(context)/4,LayoutParams.WRAP_CONTENT);
				
				if(i%4==0){
					rowLayout=new LinearLayout(context);
					rowLayout.setLayoutDirection(LinearLayout.HORIZONTAL);
				}
				...
				final SingleItemCardEntity tmpEntity=(SingleItemCardEntity)(list.get(i));
				itemTv.setText(tmpEntity.getDesc());
				...
				if(i%4==3||i==list.size()-1){
					catLayout.addView(rowLayout,rowParams);
				}
