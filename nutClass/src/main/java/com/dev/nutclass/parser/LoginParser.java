	...
	public Object parse(String jsonString){
		...
		try {
			...
			if(dataObj!=null){
				...
				UserEntity user= new UserEntity(dataObj);
				SharedPrefUtil.getInstance().setUserSession(user.buildJsonObject().toString());
				SharedPrefUtil.getInstance().setToken(token);
				SharedPrefUtil.getInstance().setMobile(dataObj.optString("mobile_phone"));
